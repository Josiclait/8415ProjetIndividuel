...
def sendToSlave(slave_index, sql):
    ...
    with SSHTunnelForwarder(slave_ips[slave_index], ssh_username="ubuntu", ssh_pkey="vockey.pem", remote_bind_address=("137.31.65.21", 3306)) as tunnel:
        return sendToMaster(sql)


@app.route("/")
def sendToCluster():
    # Flask route to receive sql message from the gatekeeper
    # Applies the proxy pattern on the requests

    type = request.json.get("implementation")
    sql = request.json.get("sql")

    if type not in ["1","2","3"]:
        return "Proxy implementation is not valid! Please choose a number between: (1) direct hit, (2) random, (3) customized"
    ...
    # Random implementation
        ...
        if(sql.lower().startswith('select')): # If input type is read, send to a slave
            ...
            return sendToSlave(slave_index, sql)
